...
bool CLikeSourceEmitter::maybeEmitParens(EmitOpInfo& outerPrec, const EmitOpInfo& prec)
{
	...
	if (isBitLogicalOrRelational(prec.leftPrecedence) && (outerPrec.leftPrecedence > kEPrecedence_Assign_Left))
		needParens = true;
	if (isBitLogicalOrRelational(outerPrec.leftPrecedence) || isBitLogicalOrRelational(outerPrec.rightPrecedence))
		needParens = true;
